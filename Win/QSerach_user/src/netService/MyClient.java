...
//		System.out.println("password:"+user.password);
	...
	public QA_class search_by_que_string(String que_string) {
		try {
			conn.send((short)6, que_string);
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("发送搜索题目字符串失败");
		}
		try {
			YuanNetPacket ynp=conn.recv();
			if(ynp.action==6) {
			...
			else if(ynp.action==7){
				System.out.println("没有在数据库中搜索到符合条件的题目");
			}else {
